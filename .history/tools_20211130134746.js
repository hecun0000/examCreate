...
const { ratio, singleChoice, multipleChoice, booleanQuestion, order } = jsonConfig
...
function createWord(list) {
  ...
  // 生成单选题
  if (list.single) {
    creatSingleQuestion(list.single, docx)
  }
  // 生成不定项选择题
  if (list.multiple) {
    creatMultipleQuestion(list.multiple, docx)
  }
  // 生成判断题
  if (list.bool) {
    creatBoolQuestion(list.bool, docx)
  }
...
function createTestByOrder (order, list, docx) {
  const orderList = {
    singleChoice: () => creatSingleQuestion(list.single, docx),
    multipleChoice: () => creatMultipleQuestion(list.multiple, docx),
    booleanQuestion: creatBoolQuestion(list.bool, docx),
  }

  Object.keys(order).forEach(keys => {
